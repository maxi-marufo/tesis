...
Calcular la pérdida de un modelo sobre una sóla muestra de prueba,
$L(\overline{h}, T_j)$ no implica promediar la pérdidad sobre ejemplos
...
error asociado a un conjunto de elementos sin etiquetar no puede ser expresado
...
elementos. Por ejemplo, para el caso binario, si en una muestra sin etiquetar
...
evaluación del error en la cuantificación es de naturaleza no lineal y
multivariada, requiriendo así la consideración conjunta de todos los elementos
no etiquetados a la vez.
...
implementación del modelo. Se requiere una colección de muestras de prueba
variada y que represente diversas distribuciones para evaluar correctamente el
rendimiento del modelo y evitar sesgos. Por esta razón, la mayoría de los
...
modelos en diferentes situaciones.
...
\subsection{Sesgo}\label{evaluacion:bias}
...
\subsection{Error Absoluto}\label{evaluacion:ae}
...
\subsection{Error Absoluto Normalizado}\label{evaluacion:nae}
...
\subsection{Error Cuadrático}\label{evaluacion:se}
...
\subsection{Error Absoluto Relativo}\label{evaluacion:rae}
...
\subsection{Error Absoluto Relativo Normalizado}\label{evaluacion:nrae}
...
\subsection{Divergencia de Kullback-Leibler}\label{evaluacion:dkl}
...
\subsection{Divergencia de Kullback-Leibler Normalizada}\label{evaluacion:ndkl}
